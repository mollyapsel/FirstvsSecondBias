...
STOP_WORDS = STOP_WORDS - set(['get', 'make', 'keep'])
dataset = load_dataset("allenai/c4", "en.noblocklist", streaming=True, split="validation")
typefr = Counter()
...
unique = typefr.keys()
m=len(all_processed_tokens)
print("Total words: ", m)

# with open('val_tokens.txt', 'w') as file:
#     file.write(str(all_processed_tokens))
# file.close()
# print("Corpus tokens saved")
...
delta_vals = [6, 6.5]
gamma_vals = [1.5, 1.75, 2]
reldf = pd.DataFrame(0, index=delta_vals, columns=gamma_vals)
simdf = pd.DataFrame(0, index=delta_vals, columns=gamma_vals)
                ...
                for i in range(0,b1):
                    for j in range(0,b2):
                        if pmiw1_sorted[i]==pmiw2_sorted[j]:
                            ...
                            betasumw1+=math.pow(pmiw2_old[pmiw1_sorted[i]],gamma)
                            betasumw2+=math.pow(pmiw1_old[pmiw1_sorted[i]],gamma)
...
socpmi(related, all_processed_tokens, delta_vals, gamma_vals, reldf)
socpmi(similar, all_processed_tokens, delta_vals, gamma_vals, simdf)

reldf = reldf/len(related)
simdf = simdf/len(similar)

reldf.to_csv('relatedc4test2.csv')
simdf.to_csv('similarc4test2.csv')
